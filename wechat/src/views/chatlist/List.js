...
class List extends Component {
  ...
  render() {
    const info_arr = Object.values(this.props.list_obj)[0]
    
    const len = Object.values(info_arr).filter(o => !o.has_read).length
    
    const last_info = info_arr[info_arr.length - 1]
    const num_style = len > 0 ? { 'display': 'block' } : { 'display': 'none' }
    return (
      <div onClick={() => {
        this.props.onChat(last_info.room_id)
      }} className='list'>
